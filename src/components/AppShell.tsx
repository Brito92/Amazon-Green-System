...
import { Home, Sprout, Store, ShoppingCart, MessageCircle, LogOut, Menu, X, ShieldCheck } from "lucide-react";
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Top bar mobile */}
      <header className="lg:hidden sticky top-0 z-40 flex items-center justify-between border-b border-border/60 bg-background/80 px-4 py-3 backdrop-blur">
        <Logo size="sm" />
      ...
      <div className="lg:flex">
        {/* Sidebar */}
        <aside
          className={`${open ? "block" : "hidden"} lg:block lg:fixed lg:inset-y-0 lg:w-64 border-r border-border/60 bg-sidebar text-sidebar-foreground`}
        >
          <div className="flex h-full flex-col p-5">
            ...
            <div className="mt-4 rounded-xl border border-sidebar-border bg-sidebar-accent/40 p-3">
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-forest text-primary-foreground text-xs font-semibold">
                  {initials}
                </div>
